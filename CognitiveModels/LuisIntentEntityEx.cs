...
{
    public static class LuisIntentEntityEx
    {
        ...
        public static string ProcessNews(this EntityModel entity)
        {
            if (entity.AdditionalProperties.TryGetValue("resolution", out dynamic resolution))
            {
                var resolutionValues = (IEnumerable<dynamic>)resolution.values;
                return resolutionValues.Select(value => value).FirstOrDefault();
            }

            throw new Exception("ProcessNews");
        }
        public static string ProcessActiveCases(this EntityModel entity)
        {
            if (entity.AdditionalProperties.TryGetValue("resolution", out dynamic resolution))
            {
                var resolutionValues = (IEnumerable<dynamic>)resolution.values;
                return resolutionValues.Select(Active => Active).FirstOrDefault();
            }

            throw new Exception("ProcessActiveCases");
        }

        public static string ProcessFailedCases(this EntityModel entity)
        {
            if (entity.AdditionalProperties.TryGetValue("resolution", out dynamic resolution))
            {
                var resolutionValues = (IEnumerable<dynamic>)resolution.values;
                return resolutionValues.Select(Death => Death).FirstOrDefault();
            }

            throw new Exception("ProcessFailedCases");
        }
    }
}
